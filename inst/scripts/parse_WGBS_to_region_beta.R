#!/usr/bin/env Rscript
#' Parse WGBS methylation call files into imprintomeR region beta tables
#'
#' This script converts common short-read WGBS methylation call formats to
#' region-level beta values over a BED-like region set. It streams each input
#' file through awk and bedtools, aggregates methylated and total counts per
#' region, combines files by sample, and writes an imprintomeR-compatible beta
#' table.
...
option_list <- list(
  make_option(c("-i", "--input"), type = "character", default = NA,
    help = "Comma-separated methylation call files or glob patterns. Positional files are also accepted."),
  make_option(c("-f", "--format"), type = "character", default = "bismark_coverage",
    help = paste(
      "Input format: bismark_coverage, bismark_cytosine, wgbstools_beta2bed,",
      "or bed_beta [default: %default]"
    )),
  ...
  make_option(c("-p", "--prefix"), type = "character", default = "WGBS_Rosenski_region",
  ...
  make_option(c("--context"), type = "character", default = "CG",
    help = "Context to keep for bismark_cytosine; use all to disable filtering [default: %default]"),
  ...
  make_option(c("--keep-temp"), action = "store_true", default = FALSE,
    help = "Keep temporary standardized region BED [default: %default]"),
  make_option(c("-v", "--verbose"), action = "store_true", default = FALSE,
    help = "Print progress messages")
)

parser <- OptionParser(
  usage = "%prog --input '*.cov.gz' --format bismark_coverage --outdir out --prefix PREFIX [options]",
  description = paste(
    "Convert common WGBS methylation call formats to region-level beta tables",
    "compatible with LoadWGBSRegionBeta() and run_imprintomeR.R -B."
  ),
  option_list = option_list,
  epilogue = paste(
    "Examples:\n",
    "  Rscript parse_WGBS_to_region_beta.R --input '*.cov.gz' --format bismark_coverage --genome hg38 -o wgbs_beta -p WGBS_Rosenski --jobs 8\n",
    "  Rscript parse_WGBS_to_region_beta.R --input '*.CX_report.txt.gz' --format bismark_cytosine --context CG -o wgbs_beta -p WGBS_CG\n",
    "  Rscript parse_WGBS_to_region_beta.R --input '*.beta.bed.gz' --format wgbstools_beta2bed --regions Rosenski_refined_iDMRs_hg19.bed -o wgbs_beta -p WGBS_hg19\n",
    "  Rscript run_imprintomeR.R -B wgbs_beta/WGBS_Rosenski_beta.tsv -m meta.tsv -o imprintome --probeset Rosenski_region --genome hg38\n",
    sep = ""
  )
)
...
valid_formats <- c("bismark_coverage", "bismark_cytosine", "wgbstools_beta2bed", "bed_beta")
...
normalization_awk <- function(input_format) {
  min_cov <- format(args$`min-site-coverage`, scientific = FALSE)
  header_skip <- '($1 ~ /^#/ || tolower($1) == "chr" || tolower($1) == "chrom") { next }'
  if (input_format == "bismark_coverage") {
    return(paste(
      header_skip,
      sprintf('{ meth = $5 + 0; unmeth = $6 + 0; cov = meth + unmeth; if (cov >= %s) print $1, $2, $3, meth, cov }', min_cov)
    ))
  }
  if (input_format == "bismark_cytosine") {
    context <- args$context
    if (tolower(context) == "all") {
      context_filter <- "1"
    } else {
      context_filter <- sprintf('toupper($6) == "%s"', toupper(context))
    }
    return(paste(
      header_skip,
      sprintf('{ meth = $4 + 0; unmeth = $5 + 0; cov = meth + unmeth; start = ($2 + 0) - 1; end = $2 + 0; if (%s && cov >= %s) print $1, start, end, meth, cov }', context_filter, min_cov)
    ))
  }
  if (input_format == "wgbstools_beta2bed") {
    return(paste(
      header_skip,
      sprintf('{ meth = $4 + 0; cov = $5 + 0; if (cov >= %s) print $1, $2, $3, meth, cov }', min_cov)
    ))
  }
  if (input_format == "bed_beta") {
    return(paste(
      header_skip,
      sprintf('{ if (NF >= 5) { meth = $4 + 0; cov = $5 + 0 } else if (NF == 4) { beta = $4 + 0; cov = 1; meth = beta } else { next }; if (cov >= %s) print $1, $2, $3, meth, cov }', min_cov)
    ))
  }
  stop("Unsupported format: ", input_format)
}
...
run_one_file <- function(file, sample_name, regions_bed, out_file) {
  ...
  norm_awk <- paste('BEGIN { FS = OFS = "\t" }', normalization_awk(args$format))
  aggregate_awk <- paste(
    'BEGIN { FS = OFS = "\t"; nreg = 7;',
