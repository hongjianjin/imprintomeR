...
The QC metadata must contain `Sample_Name`, `Basename`, and preferably `Sample_Group`. `Basename` is an IDAT filename prefix; it may be absolute or relative to `datadir`, excluding `_Red.idat` and `_Grn.idat`.
