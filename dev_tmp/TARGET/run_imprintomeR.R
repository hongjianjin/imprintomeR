...
.validate_arguments <- function(args) {
    ...
    if (is.na(args$`beeswarm-origin-max-samples`) || args$`beeswarm-origin-max-samples` <= 0) {
        errors <- c(errors, "ERROR: Invalid --beeswarm-origin-max-samples; use a positive number or Inf")
    }
...
.export_heatmap_by_gene_beta <- function(imp_set, probeset, outdir, prefix, verbose = FALSE) {

    probesets <- readRDS(system.file("extdata", "probesets_hg19.rds", package = "imprintomeR"))
    if (!probeset %in% names(probesets)) {
        warning("Cannot export heatmap_by_gene beta matrix: probeset not found: ", probeset)
    ...
    if (length(missing_cols) > 0) {
        warning("Cannot export heatmap_by_gene beta matrix: probeset missing columns: ",
                paste(missing_cols, collapse = ", "))
        return(invisible(NULL))
    }

    beta_mat <- beta(imp_set)
    meta_df <- meta(imp_set)
    common_probes <- intersect(rownames(beta_mat), probeset_df$NAME)
    if (length(common_probes) == 0) {
        warning("Cannot export heatmap_by_gene beta matrix: no probes match between beta and probeset.")
        return(invisible(NULL))
    }

    beta_sub <- beta_mat[common_probes, , drop = FALSE]
    gene_map <- setNames(probeset_df$Closest_TSS_gene_name, probeset_df$NAME)
    genes <- gene_map[common_probes]
    unique_genes <- unique(genes)

    beta_agg <- matrix(NA_real_, nrow = length(unique_genes), ncol = ncol(beta_sub))
    ...
    if (!"Sample_Name" %in% colnames(meta_df)) {
        warning("Cannot export heatmap_by_gene beta matrix: meta missing Sample_Name column.")
        return(invisible(NULL))
    }

    valid_ids <- intersect(as.character(meta_df$Sample_Name), colnames(beta_agg))
    if (length(valid_ids) == 0) {
        warning("Cannot export heatmap_by_gene beta matrix: no samples match between beta and meta.")
        return(invisible(NULL))
    }

    meta_df <- meta_df[match(valid_ids, as.character(meta_df$Sample_Name)), , drop = FALSE]
    beta_agg <- beta_agg[, valid_ids, drop = FALSE]
    if (!"Sample_Group" %in% colnames(meta_df)) {
        meta_df$Sample_Group <- "All"
    }
    meta_df <- meta_df[order(meta_df$Sample_Group), , drop = FALSE]
    beta_agg <- beta_agg[, as.character(meta_df$Sample_Name), drop = FALSE]

    out_file <- file.path(outdir, paste0(prefix, "_heatmap_by_gene.", probeset, "_beta.txt"))
    beta_out <- data.frame(Gene = rownames(beta_agg), beta_agg, check.names = FALSE)
    write.table(beta_out, out_file, sep = "\t", quote = FALSE, row.names = FALSE)

    if (verbose) {
        .log_message(paste0("  Exported heatmap_by_gene beta matrix: ", basename(out_file),
                            " (", nrow(beta_agg), " genes x ", ncol(beta_agg), " samples)"),
                     verbose = verbose)
    }
    invisible(out_file)
}
.generate_imprintome_plots <- function(imp_set, plot_types, probeset, outdir, prefix, beeswarm_origin_max_samples = 100, verbose = FALSE) {
    ...
    imp_set <- tryCatch({
        runImprintomeVisualizations(
            ...
            sample_id = sample_id,
            max_samples = beeswarm_origin_max_samples,
            prefix = prefix,
            outdir = outdir,
            save_plots = TRUE,
            store_plots = FALSE,
    ...
    if (any(plot_types %in% c("default", "all", "heatmap_by_gene"))) {
        .export_heatmap_by_gene_beta(imp_set, probeset, outdir, prefix, verbose = verbose)
    }
...
main <- function() {
    ...
    tryCatch({
        ...
        if (length(plot_types) > 0 && !args$`skip-plots`) {
            imp_set <- .generate_imprintome_plots(imp_set, plot_types, args$probeset, args$outdir, prefix = output_prefix, beeswarm_origin_max_samples = args$`beeswarm-origin-max-samples`, verbose = args$verbose)
        }
